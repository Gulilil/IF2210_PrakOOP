    ...
    try {
        for (int i = 0; i < tokens.length(); i++){
            if (IsOperand(tokens[i])){
                int operand = CharToInt(tokens[i]);
                ...
                if (opStack.empty()){
                    ...
                    // Tidak kosong
                    if (Precedence(tokens[i]) >=Precedence(opStack.top())){
                        // Prioritas lebih tinggi atau sama
                        opStack.push(tokens[i]);
                    } else {
                        // Prioritas lebih rendah
                        // Lakukan proses kalkulasi
                        while ((Precedence(tokens[i]) < Precedence(opStack.top())) && !opStack.empty()){
                            int num2 = valStack.top();
                            valStack.pop();
                            int num1 = valStack.top();
                            valStack.pop();
                            char opr = opStack.top();
                            opStack.pop();
                            if (num1 == 0 && opr == '/'){
                                throw DivideByZeroException();
                            } else {
                                int result = ApplyOp(num1, num2, opr);
        ...
        while (!opStack.empty()){
            // Kalo pembacaan selesai tapi operator stack nya masih belum empty
            int num2 = valStack.top();
            valStack.pop();
            int num1 = valStack.top();
            valStack.pop();
