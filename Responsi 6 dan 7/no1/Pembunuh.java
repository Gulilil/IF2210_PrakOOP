    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()){
            ...
            if (target.getHealth() < 20){
                tempDamage = target.getHealth();
            } else {
                tempDamage = 20;
            }
            System.out.println(self.getName()+" menyerap health "+target.getName()+" sebesar "+tempDamage);
            self.addHealth(tempDamage);
            target.terimaSerangan(tempDamage);
        } else {
