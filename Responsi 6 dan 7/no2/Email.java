public class Email {
    ...
    public boolean validateEmail() throws InvalidEmailException, InvalidDomainException {
        ...
        String[] arrStr = this.email.split("\\@");
        if (arrStr[0].isEmpty()){
            throw new InvalidEmailException("@ tidak boleh di awal email");
        } else if (arrStr[1].isEmpty()){
            throw new InvalidDomainException();
        }
        String[] arrDomain = arrStr[1].split("\\.");
        if (arrDomain[0].isEmpty()|| arrDomain.length != 2 || arrDomain[1].isEmpty() ){
            throw new InvalidDomainException();
        }
        return true;
